...
import { loadZeroSales, getDateRange, zeroSalesToCsv } from "../services/product-sales/reports.server";
import { buildDateRangeKey } from "../services/product-sales/cache.server";
import { ZeroSalesTable } from "../components/product-sales/ZeroSalesTable";
...
export const loader = async ({ request }: LoaderFunctionArgs) => {
  ...
  const { rows, cachedAt } = await loadZeroSales(admin, shopId, dateRange);
...
export const action = async ({ request }: ActionFunctionArgs) => {
  ...
  if (intent === "refresh") {
    await loadZeroSales(admin, shopId, dateRange, true);
    return null;
  }

  if (intent === "export_csv") {
    const { rows } = await loadZeroSales(admin, shopId, dateRange);
    const csv = zeroSalesToCsv(rows);
    return new Response(csv, {
      headers: {
        "Content-Type": "text/csv",
        "Content-Disposition": `attachment; filename="zero-sales-${dateRange.startDate}-${dateRange.endDate}.csv"`,
      },
    });
  }
...
export default function ZeroSalesRoute() {
  ...
  return (
    <s-stack direction="block" gap="base">
      ...
      <s-section>
        <s-stack direction="inline" gap="small" alignItems="center">
          ...
          <s-stack direction="inline" gap="small-200">
            {PRESETS.map((p) => (
              <a key={p.value} href={`/app/product-sales/zero-sales?preset=${p.value}`} style={{ textDecoration: "none" }}>
      ...
      <s-section heading="Zero Sale Products">
        <ZeroSalesTable rows={rows} dateRange={rangeKey} cachedAt={cachedAt} />
      </s-section>
    </s-stack>
  );
}
